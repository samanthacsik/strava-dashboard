...
##############################
  ...
  # filter by sport_type and select a date range ----
  filtered_sport_date <- reactive({
    acts |>
      filter(sport_type %in% input$sport) |>
      filter(start_date_local > input$date[1] & start_date_local < input$date[2])
  })

  # render elev_dist_scatterplot ----
  output$elev_dist_scatterplot <- renderPlot({

    ggplot(filtered_sport_date(), aes(x = total_miles, y = elevation_gain_ft, color = sport_type_alt, shape = sport_type_alt)) +
  ...
  output$dist_histogram <- renderPlot({
    ggplot(filtered_sport_date(), aes(x = total_miles, fill = sport_type_alt)) +
      geom_histogram() +
